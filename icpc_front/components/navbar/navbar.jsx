...
export default function Navbar({ open, hero, darkSection }) {
  ...
  // Styles
  const textColor = darkSection
    ? "text-white hover:text-gray-200"
    : "text-gray-900 hover:text-sky-600";

  const navbarBg = darkSection
  ...
  // Others dropdown (previously extra items)
  const otherLinks = [
  ...
  const navigationItems = [
    { label: "Home", href: "/", type: "link" },
    { label: "Why Amrita", href: "#perks", type: "scroll", basePath: "/" },
    // { label: "FAQ", href: "#faq", type: "scroll", basePath: "/" },
    { label: "Contest Handbook", href: "/Contest-Handbook", type: "link" },
    { label: "Hall of Fame", href: "/halloffame", type: "link" },
    { label: "Gallery", href: "/gallery", type: "link" },
    { label: "Reach Us", type: "dropdown", items: locations },
    { label: "Archive", type: "dropdown", items: archiveLinks },
    { label: "Others", type: "dropdown", items: otherLinks },
  ];

  return (
    <>
      <nav
        className={`sticky top-0 z-50 flex items-center justify-between px-[2vw] 
        ${navbarBg} min-h-[5vw] max-md:min-h-[14vw] text-[1vw] max-md:text-[4vw] 
      ...
      >
        ...
        <Link href="/" className="flex-shrink-0">
          <Image
            ...
            alt="ICPC Logo"
            unoptimized
            className="h-[6vw] max-md:h-[12vw] w-auto transition-transform hover:scale-105"
          />
        </Link>

        {/* Desktop Navigation */}
        <div className="hidden md:flex flex-1 justify-center items-center space-x-[1.5vw]">
          {navigationItems.map((item, index) => (
            <div key={index} className="relative group">
              {item.type === "link" && (
                <Link
                  href={item.href}
                  className={`px-2 py-2 ${textColor} font-medium transition duration-300 hover:bg-sky-50 rounded-lg`}
                >
                  {item.label}
                </Link>
              )}
              ...
              {item.type === "scroll" && (
                <a
                  ...
                  className={`px-2 py-2 ${textColor} font-medium transition duration-300 cursor-pointer hover:bg-sky-50 rounded-lg`}
                >
                  {item.label}
                </a>
              )}
              ...
              {item.type === "dropdown" && (
                <>
                  <button
                    className={`px-2 py-2 ${textColor} font-medium transition duration-300 flex items-center gap-1 hover:bg-sky-50 rounded-lg`}
                  >
                    {item.label}
                    <svg
                  ...
                  <div className="absolute left-1/2 -translate-x-1/2 top-full mt-2 bg-white shadow-xl rounded-lg border border-gray-200 z-20 min-w-[10vw] opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300">
                    {item.items.map((subItem, subIndex) => (
                      <Link
                        key={subIndex}
                        href={subItem.href}
                        className="block px-4 py-2 text-gray-700 hover:bg-sky-50 hover:text-sky-600 transition rounded-md font-medium"
                      >
                        {subItem.name}
                      </Link>
                    ))}
        ...
        <div className="md:hidden">
          <button
            ...
            className={`p-2 ${textColor} hover:bg-sky-50 rounded-lg transition duration-300`}
          >
            <Bars className="w-6 h-6" />
          </button>
        </div>
